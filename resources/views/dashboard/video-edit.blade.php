...
<x-app-layout>
    <x-slot name="header">
        ...
        <h4>Edit Videos( {{ $gallery->title }} )</h4>
    </x-slot>
    <div class="py-12 ">
         @if ($errors->any())
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <table>
                <tr>
                    <td>
                        <video class="rounded-btn w-50 h-30" controls>
                            <source src="/video/{{ $gallery->video }}" type="video/mp4"> 
                            <source src="/video/{{ $gallery->video }}" type="video/ogg"> 
                        </video>
                    </td>
                    <td><form class="w-full max-w-sm" method="POST" action="{{ route('videos.update',$gallery->id) }}"  enctype="multipart/form-data">
                    @csrf
                     @method('PUT')
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                   <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="inline-full-name" name="video" type="file" value="{{ $gallery->video }}">
                    </div>
                </div>
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                   <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="inline-full-name" name="link" type="file" value="{{ $gallery->video }}">
                    </div>
                </div>
